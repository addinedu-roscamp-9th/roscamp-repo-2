...
def _get_current_arm_command(client_id: str) -> Optional[Dict[str, Any]]:
    row = db_fetchone(
        """
        SELECT id, command, status, detail
        FROM arm_command_queue
        WHERE client_id=%s
          AND status IN ('CLAIMED','RUNNING','PENDING')
        ORDER BY
          CASE
            WHEN status='CLAIMED' THEN 0
...
def arm_state(
    ...
    current_cmd = _get_current_arm_command(cid)
    ...
    if row is None:
        return {
            ...
            "state": {
                ...
                "current_event": (current_cmd or {}).get("label", "--"),
            },
        }
