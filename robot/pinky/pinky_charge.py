...
# ✅ 하드코딩 목표 설정
# ==========================
HARDCODE_GOAL_X = -0.025
HARDCODE_GOAL_Y = 0.308

HARDCODE_WAYPOINT_X = 0.400
HARDCODE_WAYPOINT_Y = 0.715

NAV2_GOAL_YAW = 0.0
FINAL_YAW_CAMERA_DOWN = -math.pi / 2.0
...
class PinkyChargeClean(Node):
    """
    ✅ 운영 방식(킬/재기동 제거 버전)
    - 웨이포인트 구간: Nav2만 사용 (/cmd_vel 건드리지 않음)
    - 웨이포인트 tol 만족: Nav2 goal 확실히 cancel(완료까지 대기) -> settle -> 최종 goal
    ...
    - current_x가 ZONE_A/B/C 구간이면, 해당 zone의 via를 "웨이포인트 1개"로 먼저 방문
    - 구간에 안 걸리면 웨이포인트 스킵하고 FINAL_NAV2로 시작
    - 나머지 로직은 그대로
    """
    ...
    def __init__(self):
        ...
        # 단계
        # WP_NAV2 -> WP_SETTLE -> FINAL_NAV2 -> PID_POS -> PID_FINE -> PID_YAW -> FINISH
        self.stage = "WP_NAV2"
        ...
        # ====== (추가) START-ONCE ZONE waypoint params ======
        # 현재 위치(current_x)가 아래 구간에 속하면 via를 웨이포인트로 1회 사용
        # ZONE_A
    ...
    def amcl_callback(self, msg: PoseWithCovarianceStamped):
        ...
        # ====== (추가) 시작할 때 1회만: current_x로 via 결정 ======
        if not self._start_policy_decided:
            self._start_policy_decided = True

            # waypoint 기능 자체가 꺼져 있으면, 기존대로 FINAL로 시작
            if not self.enable_waypoint:
                self.stage = "FINAL_NAV2"
                self._nav2_goal_sent = False
                self.get_logger().info("[START_POLICY] enable_waypoint=False -> start FINAL_NAV2")
                return
            ...
            if via is not None:
                via_x, via_y = via
                # 이번 1회 웨이포인트를 zone via로 덮어쓰기
                self.waypoint_x = float(via_x)
                self.waypoint_y = float(via_y)

                self.stage = "WP_NAV2"
                self._nav2_goal_sent = False  # stage/목표가 바뀌었으니 goal send 재허용
                self.get_logger().info(
                    f"[START_POLICY] current_x={current_x:.3f} -> ZONE_{zone} via=({via_x:.3f},{via_y:.3f}) : start WP_NAV2"
                )
            else:
                # 어떤 zone도 아니면 웨이포인트 스킵하고 FINAL로 시작
                self.stage = "FINAL_NAV2"
                self._nav2_goal_sent = False
                self.get_logger().info(
                    f"[START_POLICY] current_x={current_x:.3f} -> no zone: start FINAL_NAV2"
                )
        # ===========================================================
