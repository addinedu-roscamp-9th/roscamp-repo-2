...
SERVER_BASE = os.getenv("AUTOSERVE_SERVER_BASE", "http://192.168.1.8:8000").rstrip("/")
PAYMENT_APPROVE_URL = f"{SERVER_BASE}/api/payment/approve"


def _resolve_payment_robot_id() -> str:
    explicit = (os.getenv("AUTOSERVE_ROBOT_ID", "") or "").strip()
    if explicit:
        return explicit

    domain = (os.getenv("ROS_DOMAIN_ID", "") or "").strip()
    if domain == "9":
        return "pinky1"
    if domain == "14":
        return "pinky2"
...
PAYMENT_ROBOT_ID = _resolve_payment_robot_id()
...
class MainWindow(QMainWindow):
    ...
    def on_payment_generate_and_go(self):
        ...
        saved = self._save_payment_approval(code=code, now_seoul=now_seoul)
        if saved:
            print(f"[PAYMENT] 승인코드 저장 완료 | code={code} robot_id={PAYMENT_ROBOT_ID}")
        else:
            print(f"[PAYMENT] 승인코드 저장 실패(확실하지 않음) | code={code} robot_id={PAYMENT_ROBOT_ID}")
    ...
    def _save_payment_approval(self, code: str, now_seoul: dt.datetime) -> bool:
        payload = {
            "approval_code": str(code),
            "robot_id": str(PAYMENT_ROBOT_ID),
            "status": "APPROVED",
            "src": "USER_UI",
            "note": now_seoul.isoformat(),
        }
